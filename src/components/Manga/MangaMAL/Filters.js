...
class Filters extends Component {
  ...
  renderAiringStatus = (status, text) => {
    const { publishing_statusF } = this.props.filters;
    return (
      <TouchableOpacity
        style={styles.eachradio}
        onPress={() => {
      ...
      >
        ...
        <CheckBox
          label=""
          checked={publishing_statusF.includes(status)}
          onChange={() => {
            this.props.toggleAiringStatus(status);
          }}
        />
    ...
    return (
      <View style={styles.eachradio}>
        ...
        <NumericInput
          ...
          minValue={0}
          initValue={this.props.filters.chaptersF[mode]}
        />
      </View>
    );
  };
          ...
          >
            ...
            <ScrollView style={{ padding: 20 }}>
              <View style={styles.listcontainer}>
                ...
                {this.renderListType(6, "Plan To Read")}
                {this.renderListType(2, "Completed")}
                {this.renderListType(1, "Currently Reading")}
              ...
              <View style={styles.listcontainer}>
                ...
                {this.renderAiringStatus(1, "Currently Publishing")}
                {this.renderAiringStatus(2, "Finished Publishing")}
                {this.renderAiringStatus(3, "Not Yet Published")}
                <View style={styles.divider} />
              </View>
            </ScrollView>
          </View>
...
const mapStateToProps = state => {
  return {
    ...
    filters: state.mangaMalReducer.filters
  };
};
...
const mapDispatchToProps = dispatch => {
  return {
    closeFilters: () => {
      dispatch({ type: "$CLOSE_MANGA_MAL_FILTERS" });
    },
    ...
    updateMinEpisodes: chapters => {
      dispatch({ type: "$UPDATE_MIN_EPISODES", chapters });
    },
    updateMaxEpisodes: chapters => {
      dispatch({ type: "$UPDATE_MAX_EPISODES", chapters });
    }
  };
};
